...
9. **"Should the drift_hook run in binary or trinary mode?"** Binary `δ = witness − action` is the J0/J0.5 default — the form RIAAN's `drift_hook.py` ships with. Trinary `δ = α·|intent − witness| + β·|witness − action| + γ·|intent − action|` is the J0.6+ MID-gate mode, available when a desire-extraction signal is sourced from CHARTER via the `Φ` extractor (J0.6 forge target). Binary is the projection of trinary when `intent ≡ witness` and `(α, β, γ) = (0, 1, 0)`. See [JIVA.md §5 dharma_enforcement row + §5b faculty triad](../JIVA.md) for the kit-level *why*, [`../RIAAN/MATH.md §3.4-3.5`](../../RIAAN/MATH.md) for the math, and [`../RIAAN/DESIGN.md §10b`](../../RIAAN/DESIGN.md) for the architecture.
...
| Math foundation of dual-mode dharma_enforcement (binary δ_ws + trinary δ_trishul) | [`../../RIAAN/MATH.md §3.4-3.5`](../../RIAAN/MATH.md) (trishul-drift + binary-as-projection) |
